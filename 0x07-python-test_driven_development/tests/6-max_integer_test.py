#!/usr/bin/python3
"""Unittest for max_integer([..])
"""
import unittest
max_integer = __import__('6-max_integer').max_integer

class TestMaxInteger(unittest.TestCase):
    """
    Test for the max_integer function
    """

    def test_max_integer(self):
        '''
        Testing if integer is max integer
        '''
        result = max_integer([1, 4, 3, 2])
        self.assertEqual(result, 4)
        result = max_integer([12, 9, 7, 2])
        self.assertEqual(result, 12)

    def test_isint(self):
        '''
        Testing to check variable against integer
        '''
        var = 1
        self.assertTrue(max_integer([var, 2]) == 2)
        x = 5
        self.assertTrue(max_integer([1, x]) == x)

    def test_same_entry(self):
        '''
        Testing to check max int if same
        '''
        a = 50
        self.assertEqual(max_integer([a, a, a, a]), a)

    def test_float_integer(self):
        '''
        Testing to see if float is max integer
        '''
        self.assertEqual(max_integer([4.0, 3, 2]), 4.0)

    def test_one_entry(self):
        '''
        Testing to check if only one entry
        '''
        self.assertEqual(max_integer([10]), 10)

    def test_negative_integer(self):
        '''
        Testing only negative integers
        '''
        self.assertEqual(max_integer([-1, -2, -5]), -1)
    def test_no_argument(self):
        '''
        Testing that None is returned if no argument
        '''
        self.assertEqual(max_integer(), None)
    def test_empty_list(self):
        '''
        Testing if list is empty
        '''
        self.assertEqual(max_integer([]), None)

    if __name__ == '__main__':
        unittest.main()
